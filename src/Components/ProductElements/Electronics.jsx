...
import ElectronicsData from './ElectronicsData.js';
import Electronic from "./Electronic.module.css"
import { Link } from 'react-router-dom';
const Electronics = () => {
  const Data = ElectronicsData.map((items) => (
    <Card style={{ width: '18rem', }} key={items.id}>
      ...
      <Card.Body>
        ...
        <Link to={`/Electronics/${items.id}/${items.Title}`}><Button variant="primary">Shop Now</Button></Link>
      </Card.Body>
    </Card>
  ))
  return (<>
